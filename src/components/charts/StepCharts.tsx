...
interface ActiveProcess {
  ...
  description: string;
  intensity: number; // 0-100, how active this process is right now
}

const typeColors = {
  biologisch: { bg: 'bg-green-50', border: 'border-green-200', text: 'text-green-800', badge: 'bg-green-100 text-green-700', bar: 'bg-green-400' },
  chemisch: { bg: 'bg-purple-50', border: 'border-purple-200', text: 'text-purple-800', badge: 'bg-purple-100 text-purple-700', bar: 'bg-purple-400' },
  fysisch: { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-800', badge: 'bg-blue-100 text-blue-700', bar: 'bg-blue-400' },
};
...
export function StepCharts() {
  ...
  return (
    <div className="space-y-4">
      {/* Active processes */}
      <div>
        <h3 className="text-xs uppercase tracking-wide text-stone-400 font-semibold mb-3 px-1">
        ...
        <div className="space-y-2.5">
          {processes.map((proc, i) => {
            ...
            return (
              <div key={i} className={`${colors.bg} rounded-2xl border ${colors.border}/60 p-4 shadow-sm`}>
                <div className="flex items-start gap-3">
                  <span className="text-xl flex-shrink-0 mt-0.5">{proc.icon}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`font-semibold text-sm ${colors.text}`}>{proc.name}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${colors.badge}`}>
                        {proc.type}
                      </span>
                    </div>
                    <p className="text-xs text-stone-600 leading-relaxed">{proc.description}</p>
                    {/* Intensity bar */}
