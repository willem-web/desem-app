...
export function StepCard() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Timer hero */}
      <div className={`bg-gradient-to-br ${timerGradient} text-white px-6 py-8 relative overflow-hidden`}>
        <div className="absolute top-4 right-4 bg-white/15 backdrop-blur-sm rounded-full px-3 py-1.5 text-[11px] font-semibold">
          {stepIndex + 1}/{process.stages.length}
        </div>
        <div className="text-white/70 text-sm font-medium">{definition.name}</div>
        <div className="text-[52px] font-bold font-mono tracking-tight leading-none mt-2">
          {timer.formatted}
        </div>
        ...
        {nextStage && (
          <div className="mt-3 text-white/50 text-[13px]">Hierna: {nextStage.name}</div>
        )}
      </div>
      ...
      <div className="px-5 space-y-4">
        ...
        {recipeInfo && (
          <div className="card bg-bread-50 border-bread-200">
            ...
            {recipeInfo.ingredients && (
              <div className={recipeInfo.temperature ? 'border-t border-bread-200/50 pt-4' : ''}>
                <div className="text-[10px] text-bread-600 uppercase tracking-wider font-semibold mb-2.5">Ingrediënten</div>
                {recipeInfo.ingredients.map((ing, i) => (
                  <div key={i} className="flex justify-between items-center py-1.5">
                    ...
                    <span className={`text-sm font-mono ${ing.highlight ? 'font-bold text-bread-800 bg-bread-200 px-2 py-0.5 rounded-lg' : 'text-bread-600'}`}>{ing.amount}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="card bg-lav-50 border-lav-200">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => dispatch({ type: 'COMPLETE_STRETCH_FOLD' })}
                className="px-5 py-3 bg-lav-500 text-white rounded-2xl font-semibold text-sm shadow-[var(--shadow-button)] hover:bg-lav-600">
                Voltooid
              </button>
            </div>
        ...
        {/* Instructions */}
        <div className="card">
          <h3 className="text-[10px] uppercase tracking-wider text-warm-400 font-semibold mb-4">Wat nu te doen</h3>
          <ol className="space-y-4">
            {definition.instructions.map((instruction, i) => (
              <li key={i} className="flex gap-3.5">
                <span className="flex-shrink-0 w-7 h-7 rounded-xl bg-gradient-to-br from-bread-300 to-bread-400 text-white text-[11px] font-bold flex items-center justify-center shadow-sm">
                  {i + 1}
                </span>
        ...
        {/* Expected properties */}
        <div className="card">
          <h3 className="text-[10px] uppercase tracking-wider text-warm-400 font-semibold mb-4">Wat je moet waarnemen</h3>
          <ul className="space-y-3">
            {definition.expectedProperties.map((prop, i) => (
              <li key={i} className="flex gap-3 text-[13px] text-warm-600">
        ...
        {definition.physicalTest && (
          <div className="card bg-lav-50 border-lav-200">
            <div className="flex items-center gap-2.5 mb-3">
              <div className="w-8 h-8 rounded-xl bg-lav-200 flex items-center justify-center text-sm">&#128300;</div>
              <h3 className="font-semibold text-lav-700 text-[15px]">{definition.physicalTest.name}</h3>
            </div>
            <p className="text-[13px] text-lav-600 mb-4 leading-relaxed">{definition.physicalTest.description}</p>
            <div className="space-y-2">
              {definition.physicalTest.outcomes.underfermented && (
                <div className="flex gap-3 p-3 rounded-2xl bg-red-50/60 border border-red-100">
              ...
              <div className="flex gap-3 p-3 rounded-2xl bg-green-50 border border-green-200">
              ...
              {definition.physicalTest.outcomes.overfermented && (
                <div className="flex gap-3 p-3 rounded-2xl bg-orange-50/60 border border-orange-100">
          ...
          <button onClick={() => setShowScience(!showScience)}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <span>&#129514;</span>
                <h3 className="font-semibold text-warm-700 text-[15px]">De Wetenschap</h3>
              </div>
              <span className={`text-warm-400 text-[13px] transition-transform duration-200 ${showScience ? 'rotate-180' : ''}`}>&#9660;</span>
            </div>
            {showScience && (
              <div className="mt-4 space-y-3" onClick={e => e.stopPropagation()}>
                <div className="text-[15px] font-semibold text-warm-800">{science.title}</div>
                {science.paragraphs.map((p, i) => (
                  <p key={i} className="text-[13px] text-warm-500 leading-relaxed">{p}</p>
                ))}
                {science.keyFact && (
                  <div className="p-4 bg-olive-50 rounded-2xl border border-olive-200">
                    <span className="text-[10px] font-bold text-olive-600 uppercase tracking-wider">Kernfeit </span>
                    <span className="text-[13px] text-olive-700">{science.keyFact}</span>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="pt-3 pb-8">
          <button onClick={handleNextStep}
            className={`w-full py-4.5 rounded-2xl font-bold text-base transition-all shadow-[var(--shadow-button)] ${
              timer.isComplete
                ? 'bg-gradient-to-r from-bread-400 to-bread-500 text-white hover:from-bread-500 hover:to-bread-600'
